...
from nets.IECRFNET import Segnet
import utils.misc
import utils.improc
import utils.vox
import random
...
def run_model(model, loss_fn, d, device='cuda:0', sw=None):
    ...
    _, feat_bev_e, seg_bev_e, center_bev_e, offset_bev_e, rad_bev_1, rad_bev_2, rad_bev_3, f1, f2, feat_bev, feat_bev_1, feat_bev_2 = model(
        rgb_camXs=rgb_camXs,
    ...
    if sw is not None and sw.save_this:
        ...
        sw.summ_oned('2_outputs/rad1', torch.mean(rad_bev_1, dim=1, keepdim=True))
        sw.summ_oned('2_outputs/rad2', torch.mean(rad_bev_2, dim=1, keepdim=True))
        sw.summ_oned('2_outputs/rad3', torch.mean(rad_bev_3, dim=1, keepdim=True))
        sw.summ_oned('2_outputs/f1', torch.mean(f1, dim=1, keepdim=True))
        sw.summ_oned('2_outputs/f2', torch.mean(f2, dim=1, keepdim=True))
        sw.summ_oned('2_outputs/feat_bev', torch.mean(feat_bev, dim=1, keepdim=True))
        sw.summ_oned('2_outputs/feat_bev_1', torch.mean(feat_bev_1, dim=1, keepdim=True))
        sw.summ_oned('2_outputs/feat_bev_2', torch.mean(feat_bev_2, dim=1, keepdim=True))
...
def main(
        ...
        data_dir='/root/autodl-tmp/data/nuscenes/',
        log_dir='/root/autodl-tmp/LY/logs_nuscenes/',
        init_dir='/root/autodl-tmp/LY/checkpoints/4x10_IECRNET_40-13_40000_EFFB4',
        ignore_load=None,
        # data
        res_scale=2,
        ncams=6,
